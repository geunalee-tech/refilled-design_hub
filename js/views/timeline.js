...
/* ── 구글시트 CSV 가져오기 (브라우저 세션으로 바로 Supabase 반영 — 서비스 키 불필요) ──
   시트 구조: 1행=프로젝트/업무/세부내용/담당자/진행도/월, 2행=일자, 3행부터 데이터.
   셀 텍스트 → 우리 마커 매핑. id는 이름 기반 결정적 → 재실행해도 중복 없이 갱신(멱등). */
const CSV_FROM = '2026-07-01';                 // 이 날짜 이후 마일스톤만 반영
const CSV_START = { y: 2026, m: 3 };           // 일자 컬럼 시작 월(3월)
function csvIdOf(s) { return 'c' + [...s].reduce((h, c) => ((h << 5) - h + c.charCodeAt(0)) | 0, 0).toString(36).replace('-', 'z'); }
function csvMarkerName(t) {
...
const csvTlStatus = t => /완료/.test(t) ? 'done' : /진행/.test(t) ? 'doing' : 'wait';
...
/* CSV 텍스트 → {projects:[{name, owner, subs:[{task,detail,owner,prog,ms:[{date,text}]}]}]} */
function parseSheet(text) {
  const rows = parseCSV(text);
  const dayRow = rows[2] || [];
  const DAY0 = 5;
  const colDate = [];
  let y = CSV_START.y, m = CSV_START.m, prev = 0;
  ...
  for (let r = 3; r < rows.length; r++) {
    ...
    for (let col = DAY0; col < row.length; col++) {
      ...
      const date = colDate[col]; if (!date || date < CSV_FROM) continue;
      ms.push({ date, text: v });
    }
    if (!ms.length) continue;
    if (!map.has(cur)) map.set(cur, { name: cur, subs: [] });
    map.get(cur).subs.push({ task, detail: (row[2] || '').trim(), owner: (row[3] || '').trim(), prog: (row[4] || '').trim(), ms });
  }
  return [...map.values()].filter(p => p.subs.length);
}
export function applySheet(projects) {
  const db = store.db; ensureMarkers();
  const ownerId = raw => { if (!raw) return ''; const mm = db.members.find(x => x.name && (x.name.includes(raw) || raw.includes(x.name))); return mm ? mm.id : ''; };
  let pAdd = 0, pUpd = 0, tCnt = 0;
  projects.forEach((p, pi) => {
    const pid = 'pj_' + csvIdOf(p.name);
    const ex = db.projects.find(x => x.id === pid);
    if (ex) { ex.name = p.name; ex.archived = false; if (!ex.owner) ex.owner = ownerId(p.subs[0].owner) || null; pUpd++; }
    else { db.projects.push({ id: pid, name: p.name, color: PALETTE[(db.projects.length + pi) % PALETTE.length], owner: ownerId(p.subs[0].owner) || null, source: 'csv' }); pAdd++; }
    expanded.add(pid);
    p.subs.forEach(s => {
      const sid = 'tl_' + csvIdOf(p.name + '|' + s.task);
      const milestones = s.ms.map(m => ({ date: m.date, typeId: ensureMarkerByName(csvMarkerName(m.text)) })).sort((a, b) => (a.date < b.date ? -1 : 1));
      const cur = db.tasks.find(x => x.id === sid);
      if (cur) { cur.title = s.task; cur.project = pid; cur.tlStatus = csvTlStatus(s.prog); cur.milestones = milestones; cur.assignees = ownerId(s.owner) ? [ownerId(s.owner)] : cur.assignees || []; if (s.detail) cur.notes = s.detail; }
      else db.tasks.push({ id: sid, kind: 'project', title: s.task, project: pid, assignees: ownerId(s.owner) ? [ownerId(s.owner)] : [], tlStatus: csvTlStatus(s.prog), milestones, priority: '중간', requester: '', requestedAt: CSV_FROM, due: '', link: '', files: [], notes: s.detail || '', createdAt: new Date().toISOString(), source: 'csv' });
      tCnt++;
    });
  });
  store.save();
  return { pAdd, pUpd, tCnt };
}
